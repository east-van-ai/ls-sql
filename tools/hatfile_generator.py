#!/usr/bin/env python3
"""
generate the ls-sql test corpus with the tags already in the filename.

The sister of test_data_generator.py. Same pools, same hex index, same tree.
The difference is where the words go: this one writes them into the name as
evai: tags, so the corpus is born harvested and list, query, and grep have
something to match without a harvest pass over the whole tree first.

The body is the sentence alone. The hex code is dropped from it, since the stem
already carries the code and nothing needs it twice.

Standalone on purpose. The pools and the small helpers are repeated here rather
than imported from the sister script, so either file can be lifted out and run
on its own. The cost is that a change to the word pools has to land twice.
"""
...
# The tag each digit fills. The namespace is four letters on purpose: setter.py
# reserves every two-letter namespace except ud: for the built-in harvesters.
NAMESPACE = "evai"
TAG_NAMES = ("subject", "verb", "recipient", "colour", "fruit")

# Owned by parser.py, repeated here rather than imported: tools/ is not on the
# package path, and one constant is cheaper than making it so.
SEPARATOR = "^^^"
...
def tags(code: str) -> str:
    """render a hex code as its evai: tag section."""
    return "^".join(
        f"{NAMESPACE}:{name}={word.lower()}"
        for name, word in zip(TAG_NAMES, words(code))
    )


def contents(code: str) -> str:
    """render the body of the file for a hex code: the sentence, nothing else."""
...
def leaf_parts(code: str) -> tuple[str, str, str]:
    ...
    The tree is the same two levels the plain generator builds, so the two
    corpora can be compared directory for directory.
    """
    subject, verb = words(code)[:2]
    return subject, verb, f"{subject}-{code}{SEPARATOR}{tags(code)}{SEPARATOR}.txt"
...
def survey(root: str, start: int, end: int) -> tuple[int, int, int, int]:
    ...
    would actually be written rather than the width of the range.
    """
...
def name_lengths(start: int, end: int) -> tuple[int, int]:
    """return the shortest and longest filename in the range."""
    lengths = [
        len(leaf_parts(f"{value:0{CODE_WIDTH}X}")[2]) for value in range(start, end + 1)
    ]
    return min(lengths), max(lengths)


def report(root: str, start: int, end: int, survey_result, force: bool) -> None:
    """print the counts, the name lengths, and the size estimate for a run."""
    ...
    shortest, longest = name_lengths(start, end)
    ...
    print(f"filenames  {shortest} to {longest} chars")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog=PROG,
        description="generate the ls-sql test corpus with evai: tags in the filename.",
    )
